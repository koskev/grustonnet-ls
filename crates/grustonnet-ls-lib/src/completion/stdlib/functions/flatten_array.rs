...
use grustonnet_node::{stack::NodeStack, types::{node::Node, node_kind::NodeKind, Array}};
...
impl<'a> StdLibFunction for FlattenArray<'a> {
    ...
    fn call(&self, params: Vec<Arc<Node>>) -> Result<Arc<Node>, StdLibCallError> {
        ...
        let flattened_expr = arr.elements.iter().flat_map(|elem| {
            let resolved = resolve_node(self.cache, self.document_stack, elem.expr.clone());
            if let Ok(resolved) = resolved && let NodeKind::Array(inner_arr) = resolved.node_kind.as_ref() {
